...
public abstract class CancionBase extends JPanel {
    ...
    public CancionBase(ResolucionManager resolucion) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            requestFocusInWindow(); // para que ESC funcione apenas entra
        });
    ...
    private void layoutColumnasYAnclarAbajo() {
        ...
        // Línea de hit (top de las teclas)
        yImpacto = y;
    ...
    private void verificarColision(JPanel notaUsuario) {
        ...
        for (int i = notasActivas.size() - 1; i >= 0; i--) {
            ...
            if (n.getBounds().intersects(hitUsuario)) {
                ...
                if (n.esHold) {
                    n.marcarResuelta();
                    n.marcarHeadSiNoFue(); // cabeza puntúa una sola vez
                    return;
                }
    ...
    protected void iniciarMovimientoNotas() {
        ...
        final long tiempoFinalizacionEsperadoMs = tiempoUltimaNotaMs + 4000; // colchón
    ...
    private void eliminarNotasPasadas(Nota n) {
        ...
        // Si ya fue resuelta, solo esperamos a que salga
        if (n.fueResuelta()) {
    ...
    protected void finalizarCancion() {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            RankingPanel rankingPanel = new RankingPanel(resolucion);
    ...
    // ===== Pausa unificada (sin referencias a Cancion1/Cancion4) =====
    private void alternarPausa() {
        enPausa = !enPausa;
        if (enPausa) {
            detenerMovimientoNotas();   // congela caída
            Sonido.pausarCancion();     // congela audio (sin cerrar línea)
            mostrarOverlayPausa();
        } else {
            ocultarOverlayPausa();
            Sonido.reanudarCancion();   // continúa audio
    ...
    private void mostrarOverlayPausa() {
        ...
        capaOpaca.setBackground(new Color(0, 0, 0, 110)); // negro translúcido
        ...
        panelJuego.setComponentZOrder(capaOpaca, 0); // al frente
        panelJuego.repaint();
    }
